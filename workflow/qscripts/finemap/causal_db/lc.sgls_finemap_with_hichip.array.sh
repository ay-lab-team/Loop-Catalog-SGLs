...
#PBS -N lc.sgls_finemap_with_hichip.causal_db
#PBS -d .
#PBS -V
...
# working on coverting to the SLURM mode
#row_number=( $(seq $istart $iend) )
#row_number="${row_number[$PBS_ARRAYID]}"

# getting the sample params
sample_params=( $(sed -n "${PBS_ARRAYID}p" workflow/qscripts/finemap/causal_db/lc.sgls_finemap_with_hichip.samplesheet.txt) )
